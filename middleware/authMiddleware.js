...
const fetchOrganizationInfo = async (req, res, next) => {
    if (req.session.userId) {
        try {
            const result = await db.query('SELECT u.first_name, o.organization_id, o.organization_name FROM users u JOIN organizations o ON u.organization_id = o.organization_id WHERE u.user_id = $1', [req.session.userId]);
